...
class MyHomeClient {
    ...
    // CEN+ Activation
    cenCommand(_button,_number) {
        this.command.send(sprintf("*25*21#%s*2%d##",parseInt(_button),parseInt(_number)));
    }
    ...
    onMonitor(_frame) {
        ...
        frames.forEach(function(frame) {
            ...
            /*CEN+ Activation*/
            extract = frame.match(/^\*25\*2(\d+)#(\d+)\*2(\d+)$/);
            if (extract){
                var pression = parseInt(extract[1]);
                if (pression == 1) var state = 0;
                if (pression == 2) var state = 2;
                var button = parseInt(extract[2]);
                var number = parseInt(extract[3]);
                if (this.parent != null && this.parent.onMonitor != null && typeof(this.parent.onCen) == 'function') this.parent.onCen(number,button,state);
                }
